...
public class MathDraw {
    ...
    public void plotOldInfoForTwo(final String firstOne, final String secondOne) {
        ...
        if (!resultFirstOne.isEmpty()) {
            plot.addLinePlot(String.format("Вероятность победы для %s против %s в зависимости от количества партий",
    ...
    private double[] getYPoint(final List<String> list) {
    ...
    private double[] getXPoint(final List<String> list) {
        final double[] result = new double[list.size()];
        int counter = 0;
        for (int i = 0; i < result.length; i++) {
